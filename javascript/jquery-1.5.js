...
(function(window, undefined) {
				...
				.extend({
					...
					_Deferred : function() {
						...
						deferred = {
							...
							resolve : function() {
								deferred.resolveWith(jQuery
										.isFunction(this.promise) ? this
					...
					when : function(object) {
						var args = arguments, length = args.length, deferred = length <= 1
								...
								: jQuery.Deferred(), promise = deferred
								.promise(), resolveArray;
	...
	jQuery.event = {
		...
		trigger : function(event, data, elem /* , bubbling */) {
			...
			var handle = elem.nodeType ? jQuery._data(elem, "handle") : (jQuery
					._data(elem, eventKey) || {}).handle;
	...
	jQuery.fn
			.extend({
				...
				clone : function(dataAndEvents, deepDataAndEvents) {
					dataAndEvents = dataAndEvents == null ? true
							: dataAndEvents;
	...
	var r20 = /%20/g, rbracket = /\[\]$/, rCRLF = /\r?\n/g, rhash = /#.*$/, rheaders = /^(.*?):\s*(.*?)\r?$/mg, // IE
																												...
																												// at
																												// EOL
	rinput = /^(?:color|date|datetime|email|hidden|month|number|password|range|search|tel|text|time|url|week)$/i, rnoContent = /^(?:GET|HEAD)$/, rprotocol = /^\/\//, rquery = /\?/, rscript = /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, rselectTextarea = /^(?:select|textarea)/i, rspacesAjax = /\s+/, rts = /([?&])_=[^&]*/, rurl = /^(\w+:)\/\/([^\/?#:]+)(?::(\d+))?/,
	...
	function inspectPrefiltersOrTransports(structure, options, originalOptions,
			jXHR, dataType /* internal */, inspected /* internal */) {
			...
			if (typeof selection === "string") {
				if (inspected[selection]) {
					...
					selection = inspectPrefiltersOrTransports(structure,
							options, originalOptions, jXHR, selection,
							inspected);
				}
			}
		}
		...
		if ((executeOnly || !selection) && !inspected["*"]) {
			selection = inspectPrefiltersOrTransports(structure, options,
					originalOptions, jXHR, "*", inspected);
		}
				...
				load : function(url, params, callback) {
					...
					jQuery.ajax({
						...
						complete : function(jXHR, status, responseText) {
							// Store the response as specified by the jXHR
							// object
	...
	jQuery
			.extend({
				...
				ajax : function(url, options) {
					...
					callbackContext = (s.context = ("context" in options ? options
							: jQuery.ajaxSettings).context)
							|| s, globalEventContext = callbackContext === s ? jQuery.event
					...
					loc = document.location, protocol = loc.protocol || "http:", parts,
					// The jXHR state
					state = 0,
					// Loop variable
					i,
					...
					function done(status, statusText, responses, headers) {
						...
						if (isSuccess) {
							...
							deferred.rejectWith(callbackContext, [ jXHR,
									statusText, error ]);
						}
						...
						// Complete
						completeDeferred.resolveWith(callbackContext, [ jXHR,
								statusText ]);

						if (s.global) {
							globalEventContext.trigger("ajaxComplete", [ jXHR,
									s ]);
					...
					if (!s.crossDomain) {
						...
						s.crossDomain = !!(parts && (parts[1] != protocol
								|| parts[2] != loc.hostname || (parts[3] || (parts[1] === "http:" ? 80
								: 443)) != (loc.port || (protocol === "http:" ? 80
								: 443))));
					}
					...
					if (s.beforeSend
							&& (s.beforeSend.call(callbackContext, jXHR, s) === false || state === 2)) {
						...
						if (!transport) {
							...
							// Send global event
							if (s.global) {
								globalEventContext.trigger("ajaxSend", [ jXHR,
										s ]);
							}
							// Timeout
	...
	function ajaxConvert(s, response) {
		...
		var dataTypes = s.dataTypes, converters = s.converters, i, length = dataTypes.length, tmp,
	...
	jQuery
			.ajaxPrefilter(
					"json jsonp",
					function(s, originalSettings, dataIsString /* internal */) {
					...
					if (!s.crossDomain || jQuery.support.cors) {
						...
						return {
							send : function(headers, complete) {
								...
								// #5280: we need to abort on unload or IE will
								...
								// Not set for crossDomain requests with no
								// content
								// (see why at
								...
								callback = function(_, isAbort) {
									...
									if (callback
										...
										if (isAbort) {
											...
											// Get info
											var status = xhr.status, statusText, responseHeaders = xhr
													.getAllResponseHeaders(), responses = {}, xml = xhr.responseXML;
											...
											// behaviours
											status =
											// Opera returns 0 when it should be
											// 304
											// Webkit returns 0 for failing
											// cross-domain no matter the real
											// status
											status === 0 ? (
